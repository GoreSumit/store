...
import { catchError, delay, Observable, Subject, throwError } from 'rxjs';
...
export class ProductService {
  ...
  getProducts():Observable<Products[]>{
    return this.http.get<Products[]>(`${environment.apiUrl}/products`).pipe(delay(2000),catchError(this.handleError));
  }

  getProductById(id:number):Observable<Products>{
    return this.http.get<Products>(`${environment.apiUrl}/products/${id}`).pipe(catchError(this.handleError));
